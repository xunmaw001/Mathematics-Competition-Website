    ...
    */
    ...
    public R save(@RequestBody ExamredetailsEntity examredetails, HttpServletRequest request){
        ...
        Wrapper<ExamredetailsEntity> queryWrapper = new EntityWrapper<ExamredetailsEntity>()
            ...
            .eq("examination_name", examredetails.getExaminationName())
            .eq("examredetails_types", examredetails.getExamredetailsTypes())
            ;
...
//                            examredetailsEntity.setExaminationName(data.get(0));                    //审核教师 要改的
//                            examredetailsEntity.setExamredetailsTypes(Integer.valueOf(data.get(0)));   //审核结果 要改的
//                            examredetailsEntity.setCreateTime(date);//时间
    ...
    */
    ...
    public R addUnsaved(@RequestParam Map<String, String> params, HttpServletRequest request){
        ...
        for (ExamquestionEntity examquestion:examquestionList) {
            ...
            if( state == 1){
                ...
                examredetailsEntity.setExaminationName("未作答自动批改");
                examredetailsEntity.setExamredetailsTypes(2);
    ...
    public R shenpi(Integer id,Integer fenshu,Integer jieguo, HttpServletRequest request){
        ExamredetailsEntity examredetails = examredetailsService.selectById(id);
        if(fenshu == null || fenshu < 0){
            fenshu = 0;
        }
        if(jieguo == 2){
            examredetails.setExamredetailsMyscore(fenshu);
        }else{
            examredetails.setExamredetailsMyscore(fenshu);
        }

        //多权限判断
        if(StringUtils.isNotBlank(String.valueOf(request.getSession().getAttribute("username")))){
            examredetails.setExaminationName(String.valueOf(request.getSession().getAttribute("username")));
        }else{
            examredetails.setExaminationName("匿名");
        }
        examredetails.setExamredetailsTypes(jieguo);
        boolean b = examredetailsService.updateById(examredetails);
        if(!b){
            return R.error();
        }
        Wrapper<ExamrecordEntity> queryWrapper = new EntityWrapper<ExamrecordEntity>()
                .eq("examrecord_uuid_number", examredetails.getExamredetailsUuidNumber());
        List<ExamrecordEntity> examrecord = examrecordService.selectList(queryWrapper);
        ArrayList<ExamrecordEntity> exampaperEntities = new ArrayList<ExamrecordEntity>();
        for (ExamrecordEntity e:examrecord) {
            e.setTotalScore(e.getTotalScore()+fenshu);
            if(jieguo == 2){
                ExamrewrongquestionEntity<Object> objectExamrewrongquestion = new ExamrewrongquestionEntity<>();
                objectExamrewrongquestion.setCreateTime(new Date());
                objectExamrewrongquestion.setInsertTime(new Date());
                objectExamrewrongquestion.setYonghuId(examredetails.getYonghuId());
                objectExamrewrongquestion.setExampaperId(e.getExampaperId());
                objectExamrewrongquestion.setExamquestionId(examredetails.getExamquestionId());
                objectExamrewrongquestion.setExamredetailsMyanswer(examredetails.getExamredetailsMyanswer());
                boolean insert = examrewrongquestionService.insert(objectExamrewrongquestion);
                if(!insert){
                    return R.error();
                }
            }
            exampaperEntities.add(e);
        }
        boolean b1 = examrecordService.updateBatchById(exampaperEntities);
        if(!b1){
            return R.error();
        }
        return R.ok();
